"""极简 RAG：本地 md/txt 切块 + TF-IDF 词面检索（无 embedding，便于本地学习）。"""
...
def score_chunk(query: str, chunk_text: str, idf: dict[str, float] | None = None) -> float:
...
class KnowledgeIndex:
    ...
    def search(self, query: str, top_k: int) -> dict[str, Any]:
        ...
        for ch in self._chunks:
            s = score_chunk(query, ch["text"], self._idf)
            if s > 0:
                scored.append((s, ch))
        ...
        hits = [
            {
                ...
                "score": round(s, 4),
                "text": ch["text"][:2000],  # 防止单条过大
            }
            for s, ch in top
        ]
        if not hits:
            # 无重合时退回前几条，便于演示「检索到过内容」
