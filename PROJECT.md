...
2. **工具与计算层**：`tool_registry.py` + `tools.py` + `finance_tools.py`，17 个结构化金融工具
...
- 设计 **17 个工具**：通用 3 个 + 理财/风险 11 个 + 行情/组合 5 个；`@register_tool` 注册中心
...
2. **重做会改什么？** — 向量 RAG、生产级行情缓存、更多集成测试与压测。
